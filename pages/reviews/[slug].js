...
export async function getStaticProps({ params }) {
  const filePath = path.join(process.cwd(), 'content', 'reviews', `${params.slug}.json`)
  const item = JSON.parse(fs.readFileSync(filePath, 'utf8'))
  return { props: { item } }
}

export default function ReviewPage({ item }) {
  const paragraphs = item.body ? item.body.split('\n\n').filter(Boolean) : []

  return (
    <Layout title={item.title} activePage="reviews">
      <style>{`
      ...
      <Link href="/reviews" className="back-link">← Back to Reviews</Link>
      <article>
        ...
        <div className="article-body">
          {paragraphs.length > 0 ? paragraphs.map((p, i) => <p key={i}>{p}</p>) : <p className="no-body">Full review coming soon.</p>}
        </div>
